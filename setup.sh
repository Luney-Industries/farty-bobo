...
# Generates ~/.claude/CLAUDE.md by concatenating the repo base with an optional
# user overlay at $REPO_DIR/CLAUDE.local.md (gitignored). Replaces any
# pre-existing symlink; backs up any real file whose content has diverged.
_write_claude_md() {
  local base="$REPO_DIR/CLAUDE.md"
  local overlay="$REPO_DIR/CLAUDE.local.md"
  local dst="$CLAUDE_DIR/CLAUDE.md"
  local tmp
  tmp=$(mktemp)
  {
    printf '<!-- GENERATED BY setup.sh — DO NOT EDIT. Customize via CLAUDE.local.md in the farty-bobo repo. -->\n\n'
    cat "$base"
    if [[ -f "$overlay" ]]; then
      printf '\n\n<!-- ── User customizations from CLAUDE.local.md ── -->\n\n'
      cat "$overlay"
    fi
  } > "$tmp"
  if [[ -L "$dst" ]]; then
    ...
    if ! diff -q "$dst" "$tmp" >/dev/null 2>&1; then
      ...
      cp "$dst" "$backup"
      warn "CLAUDE.md: prior file backed up to $(basename "$backup")"
    fi
  fi
  cp "$tmp" "$dst"
  rm -f "$tmp"
  if [[ -f "$overlay" ]]; then
    ok "CLAUDE.md = base + CLAUDE.local.md overlay → $dst"
  else
    ok "CLAUDE.md → $dst (create CLAUDE.local.md to add personal rules/personality)"
  fi
}
